...
def call_hf_api(client, prompt, task_type, max_retries=RETRY_ATTEMPTS):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "does not support chat_completion" in str(e).lower():
                # إذا كان الموديل لا يدعم chat_completion، استخدم text_generation
                try:
                    ...
                    logger.warning(f"⚠️ text_generation أيضاً فشل: {e2}")
            else:
