...
# -------------------- معالجة مادة النحو باستخدام Gemini (أو تأجيلها) --------------------
def process_gemini_grammar_items(items, output_file, defer=True):
    ...
    if not gemini_client:
        logger.error("❌ Gemini غير مهيأ، لن تتم معالجة عينات النحو.")
        return

    # معالجة فورية (إذا رغبت)
    # نستدعي دالة مشابهة لـ process_gemini_items من الإصدار السابق
    # لكننا سنستخدم نفس منطق استئناف العمل
    # سنضع هنا دالة قصيرة للتوضيح – في التنفيذ الحقيقي يجب تضمين الكود الكامل.
    logger.info("معالجة عينات النحو عبر Gemini (فوري) – غير مطبق في هذا الإصدار.")
    # (سنتركها لمرحلة لاحقة)
